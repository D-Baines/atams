...
/* Hub's per-variable metadata - only what Hub actually uses (see Atams::VarInfo_t for the
 * full Node-side version). NVMStorage/nvmHash are Node-only: Hub never persists variables to
 * flash itself, so it never needs to know a variable's persistence flag or migration hash. */
struct HubVarInfo_t
{
  VarType_t type           {Atams::TYPE_NULL};
  Access_t  externalAccess {Atams::ACCESS_NONE};

  /* Default Constructor - matches Atams::VarInfo_t's default-constructibility (both types'
   * fields have default member initializers), needed since Platform::NODE_NUMBER_OF_VARS may
   * be larger than a map's actual variable count (reserved headroom) - the trailing unused
   * array slots default-construct rather than needing explicit initializers. */
  HubVarInfo_t(void) = default;

  /* Direct construction - used by autogenerated user Data Block entries. */
  HubVarInfo_t(const VarType_t typeIn, const Access_t externalAccessIn) :
  type(typeIn),
  externalAccess(externalAccessIn){};

  /* Implicit and intentional - lets Hub's autogenerated code (and BlockUniversal-derived data)
   * slice a full VarInfo_t down to just the fields Hub needs, element-by-element, without
   * per-field rewrites at every call site. */
  HubVarInfo_t(const VarInfo_t &full) :
  type(full.type),
  externalAccess(full.externalAccess){};
};

using HubSharedMemoryMap_t = SharedMemoryMap_t<HubVarInfo_t>;
...
public NodeCallbackHandler
{
  ...
  struct RequestPacket_t 
  {
    ...
    uint16_t  length                                            {PACKET_HEADER_SIZE};
    WriteList writeList;
  };
  ...
  uint8_t                     nodeID_;
  const HubSharedMemoryMap_t *memoryMap_;
  ...
  bool wouldExceedResponseBuffer(const Atams::HubVarInfo_t      &varInfo,
                                 const Atams::Access_t          accessRequest,
                                 const Atams::RequestPattern_t  requestPattern,
